...
fn main() {
    ...
    //unsafe {
        ...
        //let address= ptr::null_mut();

        let process_id: isize = 10596;
        let mut process_handle: HANDLE = 0;//fn_get_current_process();
        //let process_handle: HANDLE = -1 as HANDLE;//fn_get_current_process();

        let ntdll = SyscallWrapper::new();
        // wrapper.NtAllocateVirtualMemory(hProcess, pAddress,  sSize as *mut u64, MEM_RESERVE | MEM_COMMIT, PAGE_READWRITE);
        // }
